    ...
    for (let i = 0; i < cardList.length; i += 4) {
      const row = (
        <Row className={'mb-10'} key={i}>
          {cardList[i] ? (
            <Col span={span}>
              <FlashCard key={i} card={cardList[i] as Card} />
            </Col>
          ) : undefined}
          {cardList[i + 1] ? (
            <Col span={span} offset={offset}>
              <FlashCard key={i + 1} card={cardList[i + 1] as Card} />
            </Col>
          ) : undefined}
          {cardList[i + 2] ? (
            <Col span={span} offset={offset}>
              <FlashCard key={i + 2} card={cardList[i + 2] as Card} />
            </Col>
          ) : undefined}
          {cardList[i + 3] ? (
            <Col span={span} offset={offset}>
              <FlashCard key={i + 3} card={cardList[i + 3] as Card} />
            </Col>
          ) : undefined}
        </Row>
      );
